...
def select_dialogue_column(chunk: pd.DataFrame) -> str:
    ...
    若都不存在，則回傳第一個欄位。
    """
    preferred = ["problem_response", "text", "utterance", "content", "dialogue", "Dialogue"]
...
def process_batch_dialogue(client, dialogues: list, delimiter="-----"):
    ...
    """
    prompt = (
        "你是一位針對學生學習狀況分析的專家，請根據以下編碼規則評估學生在各科目遇到的學習困難回饋，\n"
        + "\n".join(ITEMS) +
...
def main():
    ...
    output_csv = "student_obstacle_analysis.csv"
