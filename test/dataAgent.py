...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    prompt = (
        ...
        f"以下為該批次資料:\n{chunk_data}\n\n"
        "請根據以上資料進行分析，並提供學生完整的學習建議及需加強之處。"
        "其中請特別注意：\n"
        "  1. 分析學生學習結果與錯題資訊；\n"
        "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出最合適的學習方法及讀書建議(例如刷題、重點整理等)，\n"
        "     並將搜尋結果整合進回覆中；\n"
        "  3. 最後請提供具體的建議和相關參考資訊。\n"
...
async def main():
    ...
    csv_file_path = "student_progress.csv"
    chunk_size = 1000
